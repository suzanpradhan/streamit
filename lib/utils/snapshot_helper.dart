...
class SnapshotHelper<T> {
  ...
  @protected
  bool defaultLoadingPredicate(AsyncSnapshot<T> snapshot) => (snapshot.connectionState == ConnectionState.waiting) || (snapshot.connectionState == ConnectionState.active && !snapshot.hasData);

  /// [defaultDataPredicate] is the default predicate to determine the onData status
  @protected
  bool defaultDataPredicate(AsyncSnapshot<T> snapshot) => snapshot.hasData && !snapshot.hasError;
  ...
  Widget? getWidget({
    ...
    if (_loadingPredicate?.call(snapshot) ?? defaultLoadingPredicate(snapshot)) {
